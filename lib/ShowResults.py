...
def draw_optical_flow(img_old, points_old, points_current, file_save_name, sleep_for=10):
	...
	cv2.imshow('Optical Flow', img_flujo_optico)
	cv2.waitKey(sleep_for)

def draw_planar_flow(img_old, points_old, points_current, file_save_name, sleep_for=10):
	n = points_old.shape[0]
	...
	cv2.imshow('Planar Flow', img_flujo_planar)
	cv2.moveWindow('Planar Flow', 380, 0)
	cv2.waitKey(sleep_for)

def draw_points(img_old, no_match_points, match_points, file_save_name, sleep_for=10):
	...
	for i in range(n):
		...
		# Draw small circle
		cv2.circle(img_class_points, (int(x), int(y)), 3, (27,2,208), -1)
	...
	cv2.imshow('Outliers:Red', img_class_points)
	cv2.waitKey(sleep_for)

def draw_clusters(img_old, points, labels, unique_label, file_save_name, sleep_for=10):
	...
	for label in unique_label:
		...
		for point in cluster:
			...
			y = point[1]
			# Draw small circle
			cv2.circle(img_clusters, (int(x), int(y)), 3, (b,g,r), -1)

	cv2.imwrite(os.path.join('clusters', file_save_name), img_clusters)
	cv2.imshow('Clusters', img_clusters)
	cv2.waitKey(sleep_for)

def draw_box_obstacle(img_old, bounding_boxes, file_save_name, sleep_for=10):
	...
	for box in bounding_boxes:
		...
		x_max = box[2]
		y_max = box[3]
		cv2.rectangle(img_box_obs, (x_min, y_min), (x_max, y_max), (27,2,208), 2)
	cv2.imwrite(os.path.join('bboxes', file_save_name), img_box_obs)
	cv2.imshow('Obstacles', img_box_obs)
	cv2.waitKey(sleep_for)